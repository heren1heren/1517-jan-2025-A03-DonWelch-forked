...
//In Linq .FindAll can be replaced with .Where
...
//can Linq using the .Where find the first instance
// .FirstOrDefault() : returns the first instance to match the predicate OR a null/0 (default of datatype) if NO match was found
// .SingleOrDefault() : returns the first value to match the predicate OR the datatype default
foundEmployment = null;
foundEmployment = employments.Where(e => e.Title.Contains(searcharg)).FirstOrDefault();
TestForFoundItem(foundEmployment, searcharg);
